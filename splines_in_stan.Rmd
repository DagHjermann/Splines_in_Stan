...
First, here is how we create the spines: 
```{r plot_spline_2a}   
library(splines)
...
These are the first 4 splines:  
```{r plot_spline_2b, echo=FALSE, fig.height=3, fig.width=7.5, cache=TRUE}
par(mfrow = c(2,2), mar = c(4,5,2,1))
for ( i in 1:4)
  plot(X, B[,i], type="l", xlab = "x", ylab = paste("B-spline no.", i), ylim = c(0,1))
```

Here are all the splines:  
```{r plot_spline_2c, echo=FALSE, fig.height=3, fig.width=7.5, cache=TRUE}
...
Make the splines:
```{r plot_spline_4a, echo=FALSE, fig.height=3, fig.width=7.5, cache=TRUE}
B <- bs(X, knots=seq(-5,5,1), degree=3, intercept = TRUE)
dim(B)
```

These are the first 4 splines:  
```{r plot_spline_4b, echo=FALSE, fig.height=3, fig.width=7.5, cache=TRUE}
par(mfrow = c(2,2), mar = c(4,5,2,1))
for ( i in 1:4)
  plot(X, B[,i], type="l", xlab = "x", ylab = paste("B-spline no.", i), ylim = c(0,1))
```
Ande here are all the splines:  
```{r plot_spline_4c, echo=FALSE, fig.height=3, fig.width=7.5, cache=TRUE}
...
A little repetition of matrix calculus:
```{r matrix_algebra} 
a <- 1:2
B = matrix(1:6, nrow = 2)
cat("Values of a:", a, "\n")
...
cat("\n")
cat("Multiply value 1 of a with row 1 of B:\n")
a[1]*B[1,]
cat("Multiply value 2 of a with row 2 of B:\n")
a[2]*B[2,]
cat("Add both:\n")
a[1]*B[1,] + a[2]*B[2,]
...
```
Show just the first 8 components (of 25) of a random spline
...
num_basis <- nrow(B)
# First random spline: Define 25 random numbers. 25 = number of knots (21) + 2 extra on each side (since we have k = 3)  
a <- rnorm(num_basis, 0, 1)

par(mfrow = c(4,2), mar = c(3,5,1,1))
for ( i in 1:8)
  plot(X, a[i]*B[i,], type="l", xlab = "x", ylab = paste("No.", i), ylim = c(-1.5, 1.5))

```

Sum all the 25 components
```{r}
...
#
# See here:
# https://discourse.mc-stan.org/t/new-error-cleanup-makevar-old-argument-rmu-is-missing-with-no-default/18137/8
#
# Solution seemed to be rolling back withr from 2.3.0 to 2.2.0, like this (https://discourse.mc-stan.org/t/new-error-cleanup-makevar-old-argument-rmu-is-missing-with-no-default/18137/33)
# Reinstalling stan from CRAN 
# then reinstalling withr v.2.2.0, using  
#   remotes::install_version("withr", "2.2.0")
# 
# Perhaps also
#   file.rename("~/.R/Makevars.win", "~/.R/Makevars.win.bak")
# https://discourse.mc-stan.org/t/new-error-cleanup-makevar-old-argument-rmu-is-missing-with-no-default/18137/26
